-- Young Empire Unified Store Migration (Simplified)
...
-- Row Level Security (basic version without profiles table)
...
-- Authenticated users can manage products (temporary - will restrict to admins later)
CREATE POLICY "Authenticated manage products" ON merch_products 
    FOR ALL 
    TO authenticated 
    USING (true)
    WITH CHECK (true);
...
-- Authenticated users can read all orders (temporary - will restrict to admins later)
CREATE POLICY "Authenticated read all orders" ON merch_orders 
    FOR SELECT 
    TO authenticated 
    USING (true);
