...
export default function EditArtistPage() {
  ...
  useEffect(() => {
    fetch(`/api/artists/${id}`).then((r) => r.json()).then(setInitialData).catch((e) => setError(e.message)).finally(() => setLoading(false));
  }, [id]);

  const handleSubmit = async (data: any) => {
    setSaving(true); setError('');
    try {
      const res = await fetch(`/api/artists/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{initialData?.name || 'Loading...'}</p>
        </div>
      ...
      {initialData && <ArtistForm initialData={initialData} onSubmit={handleSubmit} saving={saving} />}
    </div>
  );
}
