...
export default function EditTutorialPage() {
  ...
  const handleSubmit = async (data: any) => {
    ...
    try {
      const res = await fetch(`/api/tutorials/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Edit Tutorial</h1>
      ...
      {initialData && <TutorialForm initialData={initialData} onSubmit={handleSubmit} saving={saving} />}
    </div>
  );
}
