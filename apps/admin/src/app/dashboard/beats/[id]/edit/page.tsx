...
export default function EditBeatPage() {
  ...
  useEffect(() => {
    fetch(`/api/beats/${id}`).then((r) => r.json()).then(setInitialData).catch((e) => setError(e.message)).finally(() => setLoading(false));
  }, [id]);

  const handleSubmit = async (data: any) => {
    setSaving(true); setError('');
    try {
      const res = await fetch(`/api/beats/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Edit Beat</h1>
      ...
      {initialData && <BeatForm initialData={initialData} onSubmit={handleSubmit} saving={saving} />}
    </div>
  );
}
